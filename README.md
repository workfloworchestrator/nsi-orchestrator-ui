...
A [Next.js](https://nextjs.org) front-end for **nsi-orchestrator**, the
central orchestration layer of the NSI (Network Service Interface) stack.
It wraps the published `@orchestrator-ui/orchestrator-ui-components`
library ([Elastic EUI](https://eui.elastic.co/) based) and adds the
navigation, branding, translations, and authentication wiring for the NSI
deployment.

It was forked from
[`example-orchestrator-ui`](https://github.com/workfloworchestrator/example-orchestrator-ui)
and tailored to the NSI domain: it points at the nsi-orchestrator
backend (REST + GraphQL + optional WebSocket) and ships a Helm chart for
...
- Integration with nsi-orchestrator, with sensible defaults
...
- Standalone Next.js production output, containerized and packaged as a
  Helm chart
...
flowchart LR
    ...
    UI["nsi-orchestrator-ui<br/>Next.js 15 · pages router"]
    Backend["nsi-orchestrator<br/>(REST + GraphQL + WebSocket)"]
...
```sh
git clone git@github.com:workfloworchestrator/nsi-orchestrator-ui.git
cd nsi-orchestrator-ui
...
Point the `ORCHESTRATOR_*` variables at your nsi-orchestrator instance.
...
├── chart/                     # Helm chart (nsi-orchestrator-ui)
...
`overrideMenuItems` prop on `<WfoPageTemplate>`. This repo appends a
single **Search** entry to the default menu items:
...
Add, remove, or reorder entries here to shape the navigation.
...
port `3000`. Published images live at
